...
import { type PropsType, type EmitsType, type ValueType, _getAttribute } from './checkbox-group'
...
function handClick(e: MouseEvent) {
  ...
  if (el) {
    ...
    if (!_getAttribute(el, 't-group-disabled')) {
      ...
      } else addItem(_getAttribute(el, '_value'))
    }
  }
}
function addItem(item: ValueType) {
  if (!vis.value?.includes(item)) {
    vis.value?.push(item)
  }
}
/**
...
function updateValue(val?: ValueType, immediateChange?: boolean) {
  const children: HTMLElement[] = Array.from(
    groupRef.value?.querySelectorAll<HTMLElement>('.t-checkbox[t-group]') ?? []
  )
